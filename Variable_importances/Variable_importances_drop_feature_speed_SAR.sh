...
cat > "/lustre/storeB/users/cyrilp/Svalnav/data_processing_files/PROG/Drop_MA_SAR_2018_2020_""$SGE_TASK_ID"".py" << EOF
...
path_output = '/lustre/storeB/users/cyrilp/Svalnav/April_2021/Tuning/Results_without_dayofyear/Speed/Drop_feature_training_period_SAR/'
#
...
for dat in range(0, 10):
	...
	#############################################################################################################
	filename_data = 'RF_variables_T4grid_daily_' + str(date_min) + '-' + str(date_max) + '_' + str(lead_time_start) + '-' + str(lead_time_end) + '_days.dat'
	...
	for drop_feat in range(0, len(feat_list)):
		#
		Target_train = np.array(df_RF_train['SAR_drift_magnitude'])
		Reference_train = np.array(df_RF_train['T4_drift_magnitude'])
		...
		#
		Target_test = np.array(df_RF_test['SAR_drift_magnitude'])
		Reference_test = np.array(df_RF_test['T4_drift_magnitude'])
		df_feat_test = df_RF_test.drop('SAR_drift_magnitude', axis = 1)
		df_feat_test = df_feat_test.drop('SAR_drift_initial_bearing', axis = 1)
		if feat_list[drop_feat] != 'all':
		...
		Test_forecasts = model.predict(Features_test)
		#
		...
		#
		file_output = 'MA_' + str(date_min) + '-' + str(date_max) + '_ts_' + str(Test_size) + '_' + rf_param_str + '_' + leadtime_str + '_dropfeat_' + feat_list[drop_feat] + '.dat'
...
#####################################################################################################
EOF
python3 "/lustre/storeB/users/cyrilp/Svalnav/data_processing_files/PROG/Drop_MA_SAR_2018_2020_""$SGE_TASK_ID"".py"
