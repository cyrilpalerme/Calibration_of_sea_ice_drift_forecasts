...
cat > "/lustre/storeB/users/cyrilp/Svalnav/data_processing_files/PROG/model_MA_buoys_""$SGE_TASK_ID"".py" << EOF
################################################
import matplotlib
matplotlib.use('Agg')
import os
...
path_models = '/lustre/storeB/project/copernicus/svalnav/April_2021/RF_models/Trained_with_buoys/Speed_201306_202005_without_dayofyear/'
#############################################################################################################
# Input parameters
#############################################################################################################
#
...
#
model_filename = file_input_train.replace('Buoys_T4grid_daily_', 'RF_model_MA_' + rf_param_str + '_').replace('.dat', '.pkl')
with open(path_models + model_filename, 'wb') as file:
	pickle.dump(model, file)
##############################################################################################################
EOF

python3 "/lustre/storeB/users/cyrilp/Svalnav/data_processing_files/PROG/model_MA_buoys_""$SGE_TASK_ID"".py"
