...
cat > "/lustre/storeB/users/cyrilp/Svalnav/data_processing_files/PROG/make_datasets_SAR_""$SGE_TASK_ID"".py" << EOF
...
if fraction_selected >= 0.01:
	fraction_str = str(int(100 * fraction_selected)) + '_percents'
else:
	fraction_str = str(100 * fraction_selected) + '_percents'
#############################################################################################################
# Paths 
#############################################################################################################
path_output = '/lustre/storeB/project/copernicus/svalnav/April_2021/RF_training_data/Trained_with_SAR/201801_202005_' + fraction_str + '/'
if os.path.isdir(path_output) == False:
	os.mkdir(path_output)
#
path_data = {}
path_data['SAR'] = '/lustre/storeB/project/copernicus/svalnav/Data_operational_ice_drift_forecasts/Pan_Arctic/SAR_v2/'
...
date_min = 20180104 # It must be a Thursday
date_max = 20200528
# Target variable (variable that we want to predict), syntax: source_variable
target_var_list = ['SAR_drift_magnitude', 'SAR_drift_initial_bearing']
...
###
file_output = path_output + 'RF_variables_T4grid_daily_' + str(date_min) + '-' + str(date_max) + '_' + str(lead_time_start) + '-' + str(lead_time_end) + '_days' + '.dat'
###
...
for sd in range(0, len(start_dates)):
	try:
		Target_feat_var = {}
		td_test_str = target_dates_start[sd][0:4] + '/' + target_dates_start[sd][4:6] + '/'
		file_target_var = path_data['SAR'] + td_test_str + filename['SAR'] + target_dates_start[sd] + '-' + target_dates_end[sd] + '.nc'
		exist_file = []
		exist_file.append(os.path.isfile(file_target_var)) 
		if exist_file[len(exist_file) - 1] == True:
			#
			nc_target = Dataset(file_target_var, 'r')
			status_flag_target = nc_target.variables['status_flag'][0]
			#
		...
		if (np.all(exist_file) == True):
			...
			idx_any_0 = np.logical_or(df_tar_feat['SAR_drift_initial_bearing'] == 0, df_tar_feat['SAR_drift_magnitude'] == 0)
			...
			if np.sum(idx_all == True) > 0:
				...
				N_samples = np.shape(df_tar_feat_select)[0]
				N_select = int(N_samples * fraction_selected)
				idx_select = random.sample(list(np.arange(N_samples)), N_select)
				df_tar_feat_select = df_tar_feat_select.iloc[idx_select, :]
				#
				if sd == 0:
...
python3 "/lustre/storeB/users/cyrilp/Svalnav/data_processing_files/PROG/make_datasets_SAR_""$SGE_TASK_ID"".py"
